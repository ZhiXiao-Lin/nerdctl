...
interface SpawnOptionsWithStdioLog<Stdio extends IOType | Log>
...
interface SpawnOptionsEncoding {
   ...
   */
  encoding?:
    | { stdout?: BufferEncoding; stderr?: BufferEncoding }
    | BufferEncoding;
}
...
export async function spawnFile(
  command: string,
  options: SpawnOptionsWithStdioLog<"ignore" | "inherit" | Log> &
...
export async function spawnFile(
  ...
  const encodings = [
    ...
    typeof options.encoding === "string"
      ? options.encoding
      : options.encoding?.stdout,
    typeof options.encoding === "string"
      ? options.encoding
      : options.encoding?.stderr,
  ];
  const stdStreams: [
    undefined,
  ...
  const resultMap: Record<number, "stdout" | "stderr"> = {
    1: "stdout",
    2: "stderr",
  };
